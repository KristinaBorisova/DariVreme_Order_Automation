#!/usr/bin/env python3
"""
send_order_with_quote_id_horizontal.py
Order creation for horizontal row structure (1 order + 1 client + 1 restaurant per row).
"""
...
def extract_quote_ids_from_successes(successes: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Optimized for horizontal row structure.
    """
    quote_data = []
    ...
    for success in successes:
        ...
        if quote_id:
            # All data is already in the row for horizontal structure
            row = success.get("row", {})
...
def create_order_payload(quote_data: Dict[str, Any], client_details: Dict[str, str]) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "packageDetails": {
            ...
            "description": order_details.get("order_notes", "Food delivery order"),
            "parcelValue": None,
            "weight": None,
            "products": []
        }
...
def process_orders_from_quotes_horizontal(
    ...
    print(f"🚀 Processing {len(quote_data_list)} orders with horizontal structure...")
...
def print_detailed_summary(results: Dict[str, Any]):
    ...
    print("\n" + "="*70)
    print("📊 ORDER CREATION SUMMARY (Horizontal Structure)")
    print("="*70)
    ...
    if results['successful_orders']:
        ...
        for i, order in enumerate(results['successful_orders'][:5], 1):  # Show first 5
            ...
            print(f"      Order ID: {order_details.get('order_id', 'N/A')}")
    ...
    if results['failed_orders']:
        ...
        for i, failure in enumerate(results['failed_orders'][:3], 1):  # Show first 3
            ...
            print(f"      Order ID: {original_row.get('order_id', 'N/A')}")
...
def save_order_results(results: Dict[str, Any], output_file: str = "order_results_horizontal.json"):
...
if __name__ == "__main__":
    # Example usage with horizontal structure
    
    # Option 1: Load from file (if you saved step 2 results)
    try:
        quote_results_file = "quote_results_horizontal.json"  # File from step 2
    ...
    print("\n🚀 Starting Order Creation (Horizontal Structure)")
    print("="*60)
    ...
    # Save results
    save_order_results(results, "order_results_horizontal.json")
    
    print(f"\n🎉 Order processing completed (Horizontal Structure)!")
